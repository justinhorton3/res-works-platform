...
function openPdfPage(pdf, page) {
  selectedPdfPage.value = { filename: pdf.filename, snapshotId: pdf.snapshot_id, pageNumber: page.page_number, url: `${apiBase}${page.url}`, pages: pdf.page_previews }
  pdfZoom.value = 1
  if (analysis.value?.id) fetch(`${apiBase}/projects/sweeter-build/runs/${analysis.value.id}/annotations`).then((response) => response.ok ? response.json() : []).then((items) => { annotations.value = items.map((item) => ({ ...item, pageNumber: item.page_number })) })
  nextTick(() => document.getElementById(`pdf-page-${page.page_number}`)?.scrollIntoView({ block: 'start' }))
}
...
function addPdfAnnotation(event, page) {
  ...
  const note = window.prompt(`Note for PDF page ${page.page_number}:`, '')
  if (!note?.trim()) return
  const bounds = event.currentTarget.getBoundingClientRect()
  const annotation = { id: crypto.randomUUID(), run_id: analysis.value.id, snapshot_id: selectedPdfPage.value.snapshotId, page_number: page.page_number, annotation_type: 'note', x: ((event.clientX - bounds.left) / bounds.width) * 100, y: ((event.clientY - bounds.top) / bounds.height) * 100, note: note.trim(), author: 'reviewer' }
  annotations.value.push(annotation)
  fetch(`${apiBase}/projects/sweeter-build/runs/${analysis.value.id}/annotations`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(annotation) }).catch(() => { apiError.value = 'Annotation saved locally, but could not be persisted.' })
  annotationMode.value = false
}
...
<template>
  <main class="min-h-screen bg-slate-100 text-slate-950">
    ...
    <section class="mx-auto max-w-6xl space-y-8 px-8 py-12">
      ...
      <section v-if="analysis?.result?.bundle_analysis?.pdf?.some((item) => item.page_previews?.length)" class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm"><h2 class="text-xl font-bold">PDF sheet evidence</h2><p class="mt-2 text-sm text-slate-600">Click any page to open it in the PDF viewer. Zoom, print, and review the original PDF without downloading a separate image.</p><div v-for="pdf in analysis.result.bundle_analysis.pdf" :key="`pdf-pages-${pdf.snapshot_id}`" class="mt-5"><div class="mb-3 font-semibold">{{ pdf.filename }}</div><div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"> <button v-for="page in pdf.page_previews" :key="page.page_number" type="button" class="rounded-xl border border-slate-200 p-2 text-left transition hover:border-blue-500 hover:shadow" @click="openPdfPage(pdf, page)"><img :src="`${apiBase}${page.url}`" :alt="`${pdf.filename} page ${page.page_number}`" class="w-full rounded-lg bg-slate-50" /><span class="mt-2 block text-sm font-semibold text-slate-700">Page {{ page.page_number }} · Open viewer</span></button></div></div></section>
      <Teleport to="body"><div v-if="selectedPdfPage" class="fixed inset-0 z-[9999] flex bg-slate-950" role="dialog" aria-modal="true" aria-label="PDF page viewer"><div class="flex h-screen min-w-0 flex-1 flex-col"><div class="flex shrink-0 items-center justify-between border-b border-slate-700 bg-white px-5 py-3"><div><div class="font-semibold">{{ selectedPdfPage.filename }}</div><div class="text-sm text-slate-500">All sheets · scroll to review CAD details and layout pages</div></div><div class="flex items-center gap-2"><button type="button" class="rounded px-3 py-1 text-sm font-semibold" :class="annotationMode ? 'bg-orange-500 text-white' : 'bg-slate-200 text-slate-700'" @click="annotationMode = !annotationMode">{{ annotationMode ? 'Click a sheet to place note' : 'Add annotation' }}</button><button type="button" class="rounded bg-slate-200 px-3 py-1 text-sm font-semibold text-slate-700" @click="pdfZoom = Math.max(0.5, pdfZoom - 0.25)">−</button><span class="min-w-16 text-center text-sm text-slate-600">{{ Math.round(pdfZoom * 100) }}%</span><button type="button" class="rounded bg-slate-200 px-3 py-1 text-sm font-semibold text-slate-700" @click="pdfZoom = Math.min(3, pdfZoom + 0.25)">+</button><button type="button" class="rounded bg-slate-200 px-3 py-1 text-sm font-semibold text-slate-700" @click="pdfZoom = 1">Fit</button><button type="button" class="rounded bg-slate-900 px-3 py-1 text-sm font-semibold text-white" @click="closePdfViewer">Close</button></div></div><div class="min-h-0 flex-1 overflow-auto bg-slate-700 p-6"><div v-for="page in selectedPdfPage.pages" :id="`pdf-page-${page.page_number}`" :key="page.page_number" class="mx-auto mb-6 w-fit bg-white shadow-2xl"><div class="border-b border-slate-200 px-3 py-2 text-left text-sm font-semibold text-slate-600">Page {{ page.page_number }}</div><div class="relative" :class="annotationMode ? 'cursor-crosshair' : ''" @click="addPdfAnnotation($event, page)"><img :src="`${apiBase}${page.url}`" :alt="`${selectedPdfPage.filename} page ${page.page_number}`" class="origin-top transition-transform" :style="{ transform: `scale(${pdfZoom})`, marginBottom: `${(pdfZoom - 1) * 90}%` }" /><span v-for="annotation in annotations.filter((item) => item.pageNumber === page.page_number)" :key="annotation.id" class="absolute z-10 flex h-7 w-7 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border-2 border-white bg-orange-500 text-xs font-bold text-white shadow" :style="{ left: `${annotation.x}%`, top: `${annotation.y}%` }" :title="annotation.note">{{ annotations.indexOf(annotation) + 1 }}</span></div><div v-if="annotations.some((item) => item.pageNumber === page.page_number)" class="border-t border-orange-100 bg-orange-50 px-3 py-2 text-left text-xs text-orange-900"><div v-for="annotation in annotations.filter((item) => item.pageNumber === page.page_number)" :key="`note-${annotation.id}`"><strong>{{ annotations.indexOf(annotation) + 1 }}.</strong> {{ annotation.note }}</div></div></div></div></div><aside class="hidden w-80 shrink-0 overflow-y-auto border-l border-slate-700 bg-white p-5 lg:block"><h2 class="font-semibold text-slate-900">Reviewer notes</h2><p class="mt-1 text-xs text-slate-500">Notes are linked to this run and PDF page.</p><div v-if="!annotations.length" class="mt-5 rounded-lg bg-slate-50 p-3 text-sm text-slate-500">No annotations yet. Select Add annotation, then click a sheet.</div><div v-for="annotation in annotations" :key="`rail-${annotation.id}`" class="mt-3 rounded-lg border border-orange-200 bg-orange-50 p-3 text-sm"><div class="font-semibold text-orange-900">Page {{ annotation.pageNumber }}</div><p class="mt-1 text-slate-700">{{ annotation.note }}</p><div class="mt-1 text-xs text-slate-500">{{ annotation.annotation_type || 'note' }} · {{ annotation.author || 'reviewer' }}</div></div></aside></div></Teleport>
      <div v-if="analysis?.result?.bundle_analysis?.findings?.length" class="mt-6 rounded-xl border border-orange-200 bg-orange-50 p-4"><h3 class="font-semibold text-orange-900">Review findings</h3><p class="mt-1 text-sm text-orange-800">These are evidence gaps or conflicts—not a permit-approval decision.</p><div v-for="finding in analysis.result.bundle_analysis.findings" :key="finding.message + finding.source_snapshot_id" class="mt-2 rounded-lg bg-white p-3 text-sm text-slate-700"><span class="mr-2 text-xs font-bold uppercase" :class="finding.severity === 'coverage' ? 'text-slate-500' : 'text-orange-700'">{{ finding.severity === 'coverage' ? 'coverage' : finding.severity }}</span>{{ finding.message }}<span v-if="finding.source_filename" class="mt-1 block text-xs text-slate-500">Source: {{ finding.source_filename }}</span></div></div>
      <div v-if="analysis?.result?.evidence_coverage" class="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4"><h3 class="font-semibold text-amber-900">Analysis coverage</h3><p class="mt-1 text-sm text-amber-800">Evidence is tracked by type. Available files are ready for processing; missing types are called out explicitly.</p><div class="mt-3 grid gap-2 md:grid-cols-2"><div v-for="(item, kind) in analysis.result.evidence_coverage" :key="kind" class="rounded-lg bg-white p-3"><div class="flex justify-between"><div class="font-semibold capitalize text-slate-700">{{ kind }}</div><div class="text-xs font-bold uppercase" :class="item.status === 'available' ? 'text-emerald-700' : 'text-amber-700'">{{ item.status }}</div></div><div v-if="item.sources.length" class="mt-1 text-xs text-slate-500">{{ item.sources.join(', ') }}</div><div v-if="item.optional_sources?.length" class="mt-1 text-xs text-slate-400">Optional, not analyzed: {{ item.optional_sources.join(', ') }}</div></div></div></div>
      ...
      <section v-if="analysis?.result?.bundle_analysis?.geometry?.some((item) => item.preview_url)" class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm"><h2 class="text-xl font-bold">Floor 1 SVG output</h2><p class="mt-2 text-sm text-slate-600">Architectural layers and dimensions are fitted to the Floor 1 plan view. Open the SVG in a browser tab for review; no download is required.</p><div v-for="item in analysis.result.bundle_analysis.geometry.filter((entry) => entry.preview_url)" :key="`preview-${item.snapshot_id}`" class="mt-3"><a class="font-semibold text-blue-700 underline" :href="`${apiBase}${item.preview_url}`" target="_blank" rel="noreferrer">Open {{ item.filename.replace(/\.dxf$/i, '.svg') }} preview</a></div></section>
      <section v-if="analysis?.id && sourceUrl" class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm"><h2 class="text-xl font-bold">Amended review PDF</h2><p class="mt-2 text-sm text-slate-600">Create a marked-up copy with a consolidated reviewer-notes page. The source PDF remains unchanged.</p><button type="button" class="mt-4 rounded-lg bg-orange-500 px-3 py-2 text-sm font-semibold text-white" @click="createReviewPdf">Create amended PDF</button><p v-if="reviewPdfError" class="mt-2 text-sm text-red-600">{{ reviewPdfError }}</p><a v-if="reviewPdfUrl" :href="reviewPdfUrl" target="_blank" class="mt-3 block text-sm font-semibold text-blue-700 underline">Open amended PDF</a></section>
    </section>
  </main>
</template>
