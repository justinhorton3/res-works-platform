...
<template>
  <main class="min-h-screen bg-slate-100 text-slate-950">
    ...
    <section class="mx-auto max-w-6xl space-y-8 px-8 py-12">
      ...
      <div v-if="analysis?.result?.contents_report" class="mt-6 rounded-xl border border-amber-200 bg-amber-50 p-4"><h3 class="font-semibold text-amber-900">Analysis coverage</h3><p class="mt-1 text-sm text-amber-800">CAPROJ contents are identified, but native binary geometry is not interpreted yet. Add the following Chief exports to run a substantive review:</p><div class="mt-3 grid gap-2 md:grid-cols-2"><div v-for="item in analysis.result.contents_report.requires_chief_export" :key="item.kind" class="rounded-lg bg-white p-3"><div class="font-semibold text-slate-700">{{ item.export }}</div><div class="text-xs text-slate-500">{{ item.purpose }}</div></div></div></div>
    </section>
  </main>
</template>
