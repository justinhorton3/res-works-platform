...
<template>
  <main class="min-h-screen bg-slate-100 text-slate-950">
    ...
    <section class="mx-auto max-w-6xl space-y-8 px-8 py-12">
      ...
      <section v-if="analysis?.result?.bundle_analysis?.pdf?.some((item) => item.page_previews?.length)" class="rounded-3xl border border-slate-200 bg-white p-8 shadow-sm"><h2 class="text-xl font-bold">PDF sheet evidence</h2><p class="mt-2 text-sm text-slate-600">Rendered page previews are linked to the indexed PDF source for page-level visual review.</p><div v-for="pdf in analysis.result.bundle_analysis.pdf" :key="`pdf-pages-${pdf.snapshot_id}`" class="mt-5"><div class="mb-3 font-semibold">{{ pdf.filename }}</div><div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"> <a v-for="page in pdf.page_previews" :key="page.page_number" :href="`${apiBase}${page.url}`" target="_blank" class="rounded-xl border border-slate-200 p-2"><img :src="`${apiBase}${page.url}`" :alt="`${pdf.filename} page ${page.page_number}`" class="w-full rounded-lg bg-slate-50" /><span class="mt-2 block text-sm font-semibold text-slate-700">Page {{ page.page_number }}</span></a></div></div></section>
