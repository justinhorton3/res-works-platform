...
function openPdfPage(pdf, page) {
  selectedPdfPage.value = { filename: pdf.filename, pageNumber: page.page_number }
}
...
<template>
  <main class="min-h-screen bg-slate-100 text-slate-950">
    ...
    <section class="mx-auto max-w-6xl space-y-8 px-8 py-12">
      ...
      <div v-if="selectedPdfPage" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4" role="dialog" aria-modal="true" aria-label="PDF page viewer" @click.self="closePdfViewer"><div class="flex h-[95vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><div class="flex items-center justify-between border-b border-slate-200 px-5 py-3"><div><div class="font-semibold">{{ selectedPdfPage.filename }}</div><div class="text-sm text-slate-500">Page {{ selectedPdfPage.pageNumber }} · Use the PDF viewer controls to zoom</div></div><button type="button" class="rounded-lg bg-slate-200 px-3 py-2 text-sm font-semibold text-slate-700" @click="closePdfViewer">Close</button></div><iframe :src="`${sourceUrl}#page=${selectedPdfPage.pageNumber}&zoom=page-width`" title="PDF page viewer" class="min-h-0 flex-1 w-full"></iframe></div></div>
