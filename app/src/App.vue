...
function openPdfPage(pdf, page) {
  selectedPdfPage.value = { filename: pdf.filename, pageNumber: page.page_number, url: `${apiBase}${page.url}`, pages: pdf.page_previews }
  pdfZoom.value = 1
...
function addPdfAnnotation(event, page) {
  ...
  annotations.value.push({ id: crypto.randomUUID(), pageNumber: page.page_number, x: ((event.clientX - bounds.left) / bounds.width) * 100, y: ((event.clientY - bounds.top) / bounds.height) * 100, note: note.trim() })
  annotationMode.value = false
}
