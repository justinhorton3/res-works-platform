...
function changePdfPage(offset) {
  if (!selectedPdfPage.value) return
  const index = selectedPdfPage.value.pages.findIndex((page) => page.page_number === selectedPdfPage.value.pageNumber)
  const next = selectedPdfPage.value.pages[index + offset]
  if (next) selectedPdfPage.value = { ...selectedPdfPage.value, pageNumber: next.page_number, url: `${apiBase}${next.url}` }
}
...
<template>
  <main class="min-h-screen bg-slate-100 text-slate-950">
    ...
    <section class="mx-auto max-w-6xl space-y-8 px-8 py-12">
      ...
      <div v-if="selectedPdfPage" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/75 p-4" role="dialog" aria-modal="true" aria-label="PDF page viewer" @click.self="closePdfViewer"><div class="flex h-[95vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl"><div class="flex items-center justify-between border-b border-slate-200 px-5 py-3"><div><div class="font-semibold">{{ selectedPdfPage.filename }}</div><div class="text-sm text-slate-500">Page {{ selectedPdfPage.pageNumber }} · CAD/detail sheet review</div></div><div class="flex items-center gap-2"><button type="button" class="rounded-lg bg-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-40" :disabled="selectedPdfPage.pageNumber === 1" @click="changePdfPage(-1)">Previous</button><button type="button" class="rounded-lg bg-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 disabled:opacity-40" :disabled="selectedPdfPage.pageNumber === selectedPdfPage.pages.length" @click="changePdfPage(1)">Next</button><button type="button" class="rounded-lg bg-slate-900 px-3 py-2 text-sm font-semibold text-white" @click="closePdfViewer">Close</button></div></div><div class="flex items-center justify-center gap-2 border-b border-slate-200 bg-slate-50 px-4 py-2"><button type="button" class="rounded bg-white px-3 py-1 text-sm font-semibold shadow" @click="pdfZoom = Math.max(0.5, pdfZoom - 0.25)">−</button><span class="min-w-16 text-center text-sm text-slate-600">{{ Math.round(pdfZoom * 100) }}%</span><button type="button" class="rounded bg-white px-3 py-1 text-sm font-semibold shadow" @click="pdfZoom = Math.min(3, pdfZoom + 0.25)">+</button><button type="button" class="rounded bg-white px-3 py-1 text-sm font-semibold shadow" @click="pdfZoom = 1">Fit</button></div><div class="min-h-0 flex-1 overflow-auto bg-slate-700 p-6 text-center"><img :src="selectedPdfPage.url" :alt="`${selectedPdfPage.filename} page ${selectedPdfPage.pageNumber}`" class="mx-auto origin-top transition-transform" :style="{ transform: `scale(${pdfZoom})`, marginBottom: `${(pdfZoom - 1) * 90}%` }" /></div></div></div>
